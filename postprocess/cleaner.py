import re, regex
from langdetect import detect, DetectorFactory  # language classifier
...
class CommentCleaner:
    def __init__(self):
        ...
        self.stopwords = [line.strip() for line in open('stopwords.txt', 'r').readlines()]
        self.detect_fn = detect
        self.senti_fn = pipeline(task='sentiment-analysis')
        urlpatt = "(https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9][a-zA-Z0-9-]+[a-zA-Z0-9]\.[^\s]{2,}|www\.[a-zA-Z0-9][a-zA-Z0-9-]+[a-zA-Z0-9]\.[^\s]{2,}|https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9]+\.[^\s]{2,}|www\.[a-zA-Z0-9]+\.[^\s]{2,})"
        self.urlpatt = re.compile(urlpatt)
    ...
    def clean(self, comment):
        ...
        * Split join
        * Github Emoji: :ok:, :smile:...    ^^ :)  :(  :-)  :-(
        * Remove short comments (len <= 5) with "same here/as"
        * Replace http links to "[url]"
        * Remove non-english comments:  Que dis-tu d'en crer une issue pour le faire plus tard ? ^^'
        * Positive sentiments: Ah, nice...  This is an improvement!, This makes for a good review tip!
        * (This is not removed now.) User link: @user @ianwremmel @link
        """
        if comment == "":
            return comment
        if self.containCodes(comment):
            return "<CODEBLOCK>"
        comment = self.cleanString(comment)
        comment = self.removeEmojis(comment)
        # a lot of weird symbols found while processing
        comment = self.removeStrange(comment)
        # comment = self.removeStopwords(comment)
        ...
        if not self.isCommentRelevant(comment):
            return "<IRRELEVANT>"
        comment = self.removeUrl(comment)
        if not self.isEnglish(comment):
            return "<NONENG>"
        if len(comment.split()) < 10 and self.isPositve(comment):
            return "<POSITIVE>"
        return comment
    
    @staticmethod
    def containCodes(comment):
        regex = r"```[a-zA-Z0-9]*\n?[\s\S]*?\n?```"
        return re.search(regex, comment) is not None

    def isPositve(self, comment):
        result = self.senti_fn(comment)[0]
        return result['label'] == "POSITIVE" and result['score'] > 0.99
    ...
    @staticmethod
    def removeStrange(s):
        s = s.replace("←", "<-").replace("१२", "").replace("⁻⁵", "").replace("•", "") \
    ...
    def isEnglish(self, text):
        try:
            lang = self.detect_fn(text) == "en"
        except:
            lang = True
        return lang

    def removeStopwords(self, text):
        text = ''.join(w + ' ' for w in text.split() if w.lower() not in self.stopwords).strip()
        if text.endswith(("?", "!", ".")):
            text = text[:-1]
        if text.startswith(('-', '.', ':', '>')):
            text = text[1:].strip()
...
class DiffCleaner(CommentCleaner):
    def __init__(self) -> None:
        super().__init__()
    
    @staticmethod
    def extract_from_diff(s):
        lines = [line for line in s.split("\n")[1:] if len(line.strip()) > 0]
        orig_lines = [line for line in lines if line[0] != "+"]
    ...
    @staticmethod
    def cleanDiff(diff: str):
        if len(diff.split("\n")) >= 20:
            return "<LONG>", "<LONG>"
        olds, news = DiffCleaner.extract_from_diff(diff)
        if " ".join(olds.split()) == " ".join(news.split()):
            return "<SAME>", "<SAME>"
        return olds, news
    
    @staticmethod
    def containCodes(comment):
        # do not remove code blocks
        return False

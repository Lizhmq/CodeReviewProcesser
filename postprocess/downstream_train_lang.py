...
clsfiles = [repo2str(repodic, "cls") for repodic in repos if repodic["lang"] in [".cs", "java", "rb"]]  # !!!!!!!!! langs here
print(clsfiles[:10])
print(len(clsfiles))
...
for clsfile, genfile in tqdm(zip(cls_files, gen_files)):
    ...
    if len(outdata) > breakcnt:
        with open(os.path.join(outputdir, f"outtrain-lang-{chunkidx}.jsonl"), "w") as f:
            for data in outdata:
...
with open(os.path.join(outputdir, f"outtrain-lang-{chunkidx}.jsonl"), "w") as fp:
    for data in outdata:
